...
clear
echo "$(date '+%Y-%m-%d %H:%M:%S') - Starting Elasticsearch setup script on node 2."

# Prompt user for IP addresses of node-1 and node-3
read -p "Enter IP address for node-1: " NODE1_IP
echo "You entered: $NODE1_IP"
...
discovery.seed_hosts: ["$CURRENT_IP", "$NODE1_IP", "$NODE3_IP"]

# Bootstrap the cluster using initial master nodes
cluster.initial_master_nodes: ["node-1", "node-2", "node-3"]

# Node roles (assign the node as master)
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - Elasticsearch setup script completed successfully."
