...
clear
echo "$(date '+%Y-%m-%d %H:%M:%S') - Starting Elasticsearch setup script on node 3."

# Prompt user for IP addresses of node-1 and node-2
...
discovery.seed_hosts: ["$CURRENT_IP", "$NODE1_IP", "$NODE2_IP"]
...
echo "$(date '+%Y-%m-%d %H:%M:%S') - Elasticsearch node-3 setup script completed successfully."
